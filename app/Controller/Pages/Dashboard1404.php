...
class Dashboard1404 extends ViewComponents
{
    ...
    public static function getDataViewDash($request)
    {
        ...
        if ($isSuperAdmin) {
            ...
            // Verifica se o objeto existe E se a data não é vazia
            $currentPix = ($dataPix && isset($dataPix->value))
                ? str_replace('.', ',', sprintf("%0.2f", (float)$dataPix->value))
                : '00,00';

            $currentData = ($dataPix && !empty($dataPix->confirmed_date) && strtotime($dataPix->confirmed_date))
