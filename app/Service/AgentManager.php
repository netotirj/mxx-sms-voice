...
class AgentManager
{
    ...
    /*public function addFromPayload(array $endpoints, bool $forceOnline = false): array
    {
        ...
        foreach ($endpoints as $raw) {
            ...
            if (!$this->isValidRamal($ramal)) continue;

            $rawInfo = $this->redis->hget($this->agentsKey, $ramal);
            $info = $rawInfo ? json_decode($rawInfo, true) : [];

            $online = $forceOnline || (bool)($this->checkEndpointOnline($ramal) ?? false);

            // (opcional) debug
            // var_export($online);

            if (!$online) {
                // 🔴 OFFLINE: limpa tudo que pode “prender”
                $info['status'] = 'OFFLINE';
                $info['online'] = false;
                ...
                // se estava OFFLINE, volta pra LIVRE
                if (($info['status'] ?? null) === 'OFFLINE') {
                    $info['status'] = 'LIVRE';
                }

                // normaliza reserved_by: se for vazio/null, remove a chave
                $rb = $info['reserved_by'] ?? null;
                if ($rb === null || $rb === '' || $rb === false) {
                    unset($info['reserved_by']);
                }

                // se não tem status definido, garante LIVRE
            ...
            $this->redis->hset($this->agentsKey, $ramal, json_encode($info, JSON_UNESCAPED_UNICODE));
            $this->publish('agent:updated', ['ramal' => $ramal, 'info' => $info]);
    ...
    public function addFromPayload(array $endpoints, bool $forceOnline = false): array
    {
        ...
        foreach ($endpoints as $raw) {
            ...
            $st = $this->checkEndpointOnline($ramal); // ?bool
            if ($forceOnline) $st = true;
            ...
            if ($st === false) {
                ...
                // ⚪ st === null -> ARI indisponível/auth/timeout/etc
                $info['ari_check'] = 'unknown';
                $info['ari_check_ts'] = $now;
                $info['stale'] = true;

                // ✅ NÃO inventa OFFLINE
                // mas se o último OK já é antigo, aí sim vira UNKNOWN
                $lastOk = (int)($info['ari_last_ok_ts'] ?? 0);
                if ($lastOk > 0 && ($now - $lastOk) > 30) { // ajuste: 30s/60s
                    $info['online'] = null;
                    $info['status'] = 'UNKNOWN';
                }

                // se não existe status ainda, garante UNKNOWN
                if (empty($info['status'])) {
                    $info['online'] = $info['online'] ?? null;
                    $info['status'] = 'UNKNOWN';
                }
            }
