...
class PermissionsRules
{
    ...
    public static function userCanAccessRoute(int $userId, string $routeName, string $tenancyId): bool
    {
        ...
        if (!$user || empty($user['user_function'])) {
            return false;
        }
    ...
    // ============================================
    // 🔹 NOVO: Sincroniza permissões para SUPER ADMIN
    // ============================================
    public static function syncSuperAdminPermissions(int $userId): void
    {
        $db = new Database();

        // 1️⃣ Busca todas as permissões globais (sem tenancy)
        $allPermissions = $db->execute("
            SELECT id AS permission_id, route_name
            FROM permissions p
            LEFT JOIN permission_routes pr ON pr.permission_id = p.id
            WHERE p.tenancy_id IS NULL
        ")->fetchAll(PDO::FETCH_ASSOC);

        // 2️⃣ Cria overrides globais (sem tenancy, sem role)
        foreach ($allPermissions as $perm) {
            if (empty($perm['route_name'])) continue;

            $db->execute("
                INSERT IGNORE INTO role_permission_overrides 
                    (tenancy_id, user_id, role_id, permission_name, is_enabled)
                VALUES (NULL, :user_id, NULL, :permission_name, 1)
            ", [
                ':user_id' => $userId,
                ':permission_name' => $perm['route_name']
            ]);
        }
    }
