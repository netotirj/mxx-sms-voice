...
class AgentManager
{
    ...
    public function addFromPayload(array $endpoints, bool $publish = true): array
    {
        ...
        foreach ($endpoints as $ramal) {
            ...
            $finalOnline = ($isOnline === null) ? ($current['online'] ?? false) : $isOnline;

            // 3. BLOQUEIO DE SEGURANÇA (Soberania do Usuário)
            $statusNoRedis = strtoupper((string)($current['status'] ?? 'OFFLINE'));
            $motivoPausa   = $current['status_name'] ?? null;

            /**
             * 🛡️ TRAVA DEFINITIVA:
             * Se o usuário acionou a pausa (tem status_name ou status PAUSA),
             * o sistema NÃO PODE mudar para LIVRE ou OFFLINE por conta própria.
             * Ele só sai da PAUSA se o usuário clicar no botão (o que limparia o status_name).
             */
            if (!is_null($motivoPausa) || $statusNoRedis === 'PAUSA' || $statusNoRedis === 'OCUPADO') {
                // Mantém o status de negócio, independente do WebRTC estar online ou não agora
                $statusParaGravar = ($statusNoRedis === 'OCUPADO' && $finalOnline) ? 'OCUPADO' : 'PAUSA';
            } else {
                // Se o usuário NÃO está em pausa, aí sim o sistema decide entre LIVRE ou OFFLINE
                $statusParaGravar = $finalOnline ? 'LIVRE' : 'OFFLINE';
            }
            ...
            $payload = [
                ...
                'status_since'   => $current['status_since'] ?? $now,
