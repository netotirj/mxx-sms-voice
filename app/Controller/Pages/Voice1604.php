...
class Voice1604 extends ViewComponents
{
    ...
    public static function setVoiceHangup($request): Response
    {
        ...
        try {
            $client = new Client(['auth' => ['maxx', 'mxx123'], 'timeout' => 5]);
            $ariBase = 'http://192.168.1.9:8088/ari/';
    ...
    public static function setVoiceAnswer($request): Response
    {
        ...
        if (!$callId) {
            return new Response(400, json_encode(['success' => false, 'message' => 'call_id ausente']), 'application/json');
        }
        ...
        try {
            ...
            $resp = $client->post("{$ariBase}channels/{$callId}/answer");

            $status = $resp->getStatusCode();
            $body   = (string) $resp->getBody();

            if ($status >= 200 && $status < 300) {
                return new Response(200, json_encode(['success' => true]), 'application/json');
            }
    ...
    public static function getAudiosFilesAsterisk(): Response
    {
        ...
        $baseUrl = 'http://192.168.1.9/audio.php';
    ...
    /*public static function processCdrFromRedis(): void
    {
        try {
            ...
            //$cdr->endpoint = $tariff['endpoint'] ?? null;

            static $userCache = []; // 🔥 FORA do while
            ...
            while (true) {
                ...
                // ============================
                // 🔹 BUSCA SNAPSHOT DO USER
                // ============================
                if (!empty($cdr->user_id)) {

                    $uid = (int)$tariff['owner_id'];;

                    if (!array_key_exists($uid, $userCache)) {

                        $u = UserAuthentication::getUserById($uid);

                        $userCache[$uid] = $u ? [
                            'name' => $u->name ?? null,
                            'account_code' => $u->account_code ?? null,
                        ] : null;
                    }

                    if ($userCache[$uid]) {
                        $cdr->user_name         = $userCache[$uid]['name'];
                        $cdr->user_account_code = $userCache[$uid]['account_code'];
                    }
                }



                $cdr->endpoints = $tariff['endpoint'] ?? null;
                ...
                // ==========================================================
                // 🎙️ INDEXAÇÃO DA GRAVAÇÃO NO BANCO (COM TRAVA DE RAMAL)
                // ==========================================================
                if ($cdr->dialstatus === 'ANSWER' && !empty($cdr->call_id)) {

                    // 🛡️ O PULO DO GATO: Verificamos se NÃO é ramal antes de seguir
                    $isInternal = self::isRamalCdr($cdr->channel_number, $cdr->number, $cdr->destination);

                    if (!$isInternal) {

                        $ccRaw = $redis->get("voice:call_context:{$cdr->call_id}");

                        if ($ccRaw) {
                            $cc = json_decode($ccRaw, true);
                            if ((int)($cc['record_calls'] ?? 0) === 1) {

                                $fileName = "{$cdr->call_id}.wav";
                                // Caminho absoluto corrigido conforme a sua estrutura
                                ...
                                // 🚀 Só indexa o áudio para o cliente final (Perna Externa)
                                (new AsteriskExtensionsSip())->createAudio($audioQuery, $audioPayload);

                                echo " [AUDIO-INDEX] ✅ Sucesso (Destino Externo): {$fileName}\n";
                            }
                        }
                    } else {
                        echo " [AUDIO-INDEX] ⏭️ Pulando indexação: Perna identificada como RAMAL.\n";
                    }
                }
    ...
    public static function processCdrFromRedis(): void
    {
        try {
            ...
            while (true) {
                ...
                // fila vazia: não encerra erro, só aguarda
                if (!$json) {
                    usleep(200000); // 200ms
                    break;
                }
                ...
                // Remove techprefix do destino, se vier embutido
                if (
                    !empty($cdr->destination) &&
                    !empty($cdr->techprefix) &&
                    str_starts_with($cdr->destination, $cdr->techprefix)
                ) {
                    $cdr->destination = substr($cdr->destination, strlen($cdr->techprefix));
                }

                // =====================================================
                // 🔹 SNAPSHOT DO USUÁRIO (name + account_code)
                // =====================================================
                if (!empty($cdr->user_id)) {
                    $uid = (int)$cdr->user_id;
                ...
                // =====================================================
                // 🚫 IGNORAR CDR COM VALOR <= 0 (ramal não cobra)
                // =====================================================
                if ($cdr->value <= 0) {
                    continue;
                }
                ...
                // =====================================================
                // 🔥 REGRAS DE DÉBITO BASEADAS EM role/type
                // =====================================================
                $adminId = RegisterTenancies::getTenancyOwnerUserId($cdr->tenancy_id);
                ...
                // =====================================================
                // 🔹 RESELLER → debita reseller + admin pai
                // =====================================================
                if ($cdr->role === 'reseller') {
                    // A) debita reseller_balance com valor do Asterisk
                    BalanceSms::decrementResellerBalance(
                        $cdr->value,
                        $cdr->user_id,
                        $cdr->tenancy_id
                    ...
                    // =====================================================
                    // 🔹 USUÁRIO NORMAL → debita apenas o user
                    // =====================================================
                    $userBalanceObj = BalanceSms::getBalanceSms($cdr->user_id, $cdr->tenancy_id);
